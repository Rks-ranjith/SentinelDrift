...
function generateExclusionSnippet(threat: ThreatEvent): string {
  const ruleName = threat.matched_rules && threat.matched_rules.length > 0 
    ? threat.matched_rules[0] 
    : "*"
  
  return `RuleExclusion(
    rule_name="${ruleName}",
    ips=["${threat.source_ip}"],
    paths=["${threat.request_path}"]
)`
}
...
export function ThreatDetail({
  ...
  const handleCopy = async () => {
    const snippet = generateExclusionSnippet(threat)
    try {
      ...
      >
        ...
        <div className={styles.body}>
          <div className={styles.actionsBar}>
            ...
            >
              {showSnippet ? 'Hide Bypass Rule' : 'Generate Bypass Rule'}
            </button>
          </div>
          ...
          {showSnippet && (
            <section className={`${styles.section} ${styles.snippetSection}`}>
              <div className={styles.snippetHeader}>
                <h3 className={styles.sectionTitle}>Backend Exclusion Snippet</h3>
                <button 
                  type="button" 
                ...
                >
                  {copied ? <LuCheck className={styles.textSuccess} /> : <LuCopy />}
                  {copied ? 'Copied!' : 'Copy'}
                </button>
              </div>
              <pre className={styles.codeSnippet}>
                {generateExclusionSnippet(threat)}
              </pre>
              <p className={styles.snippetHelp}>
                Paste this into your <code>RuleEngine</code> exclusions list in the backend configuration to whitelist this specific IP and path.
              </p>
            </section>
          )}
