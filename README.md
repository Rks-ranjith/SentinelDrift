...
> [!NOTE]
> **Attribution:** This project (SentinelDrift) is a custom fork and educational rework of the original [AI-Powered Threat Detection Engine](https://github.com/CarterPerez-dev/Cybersecurity-Projects/tree/main/PROJECTS/advanced/ai-threat-detection) from the [CarterPerez-dev/Cybersecurity-Projects](https://github.com/CarterPerez-dev/Cybersecurity-Projects) repository. Key changes and improvements are documented in the [Changelog](#changelog) section below.
...
### [2026-07-09] - Rule Bypass & Project Attribution
- **Rule Exclusions & Whitelisting**: Implemented path-based, IP-based, and global (`*`) rule bypass capability in `RuleEngine` to handle false positives in production environments.
- **Unit Tests**: Added comprehensive test cases verifying whitelisting functionality in `test_detection.py`.
- **Codebase Attribution Cleanup**: Removed all inline per-file author tags (such as `©AngelaMos` / `AngelaMos`) from comments and docstrings across the codebase (including configuration, docker files, HTML metadata, and scripts) to streamline and personalise the learning fork.
